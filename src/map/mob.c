...
struct mob_data *mob_once_spawn_sub(struct block_list *bl, int m, short x, short y, const char *mobname, int class_, int hp_per, const char *event)
{
...
int mob_once_spawn(struct map_session_data* sd, int m, short x, short y, const char* mobname, int class_, int amount, int hp_per, int minmax, int killtime, const char* event)
{
	...
	for (count = 0; count < amount; count++)
	{
		...
		md = mob_once_spawn_sub(sd?&sd->bl:NULL, m, x, y, mobname, c, hp_per, event);
...
int mob_clone_spawn(struct map_session_data *sd, int m, int x, int y, const char *event, int master_id, int mode, int flag, unsigned int duration)
{
	...
	//Finally, spawn it.
	md = mob_once_spawn_sub(&sd->bl, m, x, y, "--en--",class_,100,event);
